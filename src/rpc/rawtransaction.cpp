...
void ScriptPubKeyToJSON(const CScript& scriptPubKey, UniValue& out, bool fIncludeHex)
{
    ...
    BOOST_FOREACH(const CTxDestination& addr, addresses)
        a.push_back(CIlcoinAddress(addr).ToString());
...
void SmartContractToJSON(const CTransaction& tx, const uint256 hashBlock, UniValue& entry)
{
    ...
    if (isSmartContract) {
      ...
      sc_out.push_back(Pair("txid_list", smartcontract_json.value("txid_list", "")));
...
UniValue getrawsmartcontract(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            "getrawsmartcontract \"txid\" ( verbose )\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("getrawsmartcontract", "\"mytxid\"")
            + HelpExampleCli("getrawsmartcontract", "\"mytxid\" true")
            + HelpExampleRpc("getrawsmartcontract", "\"mytxid\", true")
        );
    ...
    CTransactionRef tx;
    uint256 hashBlock;
    if (!GetSmartContract(hash, tx, Params().GetConsensus(), hashBlock, true))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string(fTxIndex ? "No blockchain smart contract"
            : "No blockchain smart contract. Use -txindex to enable blockchain smart contract queries") +
            "");
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the ilcoin address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) ilcoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue decodescript(const JSONRPCRequest& request)
{
    ...
    if (type.isStr() && type.get_str() != "scripthash") {
        ...
        r.push_back(Pair("p2sh", CIlcoinAddress(CScriptID(script)).ToString()));
    }
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "rawtransactions",    "getrawsmartcontract",    &getrawsmartcontract,    true,  {"txid","verbose"} },
