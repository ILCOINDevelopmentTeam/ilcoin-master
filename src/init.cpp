...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    if (fReindex) {
        ...
        while (true) {
            ...
            totalLoaded = totalLoaded + LoadExternalBlockFile(chainparams, file, &pos, totalLoaded);
            nFile++;
        }
    ...
    if(nHeight >= 218018){
      ...
      CValidationState state;
      if (!ActivateBestChain(state, chainparams, true)) {
          LogPrintf("Failed to connect best block");
          StartShutdown();
      }
    }
    ...
    int nShift = (0x1d00ffff >> 24) & 0xff;
    double dDiff =
        (double)0x0000ffff / (double)(0x1d00ffff & 0x00ffffff);
    while (nShift < 29)
    {
        dDiff *= 256.0;
        nShift++;
    ...
    LogPrintf("*** chainActive.Tip() dDiff: %d\n",dDiff);
