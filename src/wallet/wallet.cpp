...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CIlcoinAddress(CScriptID(redeemScript)).ToString();
...
void CWallet::ReacceptWalletTransactions()
{
    ...
    BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
    {
        ...
        // LogPrintf("CWallet::ReacceptWalletTransactions wtx.GetHash(%s) Depth(%d) OK!\n", wtx.GetHash().ToString(), nDepth);
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl* coinControl, bool sign)
{
    ...
    // JCOL commitment
    if (GetRandInt(10) == 0)
        txNew.nLockTime = 0;//std::max(0, (int)txNew.nLockTime - GetRandInt(100));
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-ilcoin-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CIlcoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CIlcoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CIlcoinAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CIlcoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CIlcoinAddress(address).ToString());
}
 ...
 * https://ilcointalk.org/?topic=54527, or
 * https://github.com/ilcoin/ilcoin/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CIlcoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CIlcoinAddress(dest).ToString(), key);
}
...
int CMerkleTx::GetDepthInMainChain(const CBlockIndex* &pindexRet) const
{
    ...
    // LogPrintf("CMerkleTx::GetDepthInMainChain hashBlock(%s) OK!\n", hashBlock.ToString());
