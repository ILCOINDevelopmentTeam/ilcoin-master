...
void CTxMemPool::removeUnchecked(txiter it, MemPoolRemovalReason reason)
{
    ...
    LogPrintf("CTxMemPool::removeUnchecked size (%lu) OK!\n", mapTx.size());
}
...
void CTxMemPool::removeForBlock(const std::vector<CTransactionRef>& vtx, unsigned int nBlockHeight)
{
    ...
    int c = 0;
    LogPrintf("CTxMemPool::removeForBlock size (%lu) OK!\n", mapTx.size());
    for (const auto& tx : vtx)
    {
        ...
        indexed_transaction_set::iterator i2 = mapTx.end();
        if (i != i2){
          LogPrintf("CTxMemPool::entries.push_back enters OK!\n");
          entries.push_back(&*i);
        }
    }
    ...
    for (const auto& tx : vtx)
    {
        ...
        LogPrintf("CTxMemPool::ClearPrioritisation size (%lu) hash(%d) %s OK!\n", mapTx.size(), (c), tx->GetHash().ToString());
    }
