...
uint64_t ReadCompactSize(Stream& is)
{
    ...
    else if (chSize == 253)
    {
        ...
        if (nSizeRet < 253)
            throw std::ios_base::failure("(1) non-canonical ReadCompactSize()");
    }
    else if (chSize == 254)
    {
        nSizeRet = ser_readdata32(is);
        if (nSizeRet < 0x10000u)
            throw std::ios_base::failure("(2) non-canonical ReadCompactSize()");
    }
    else
    {
        nSizeRet = ser_readdata64(is);
        if (nSizeRet < 0x100000000ULL)
            throw std::ios_base::failure("(3) non-canonical ReadCompactSize()");
    }
