...
static const char *MSG_HASHMINIBLOCK = "hashminiblock";
...
bool CZMQPublishHashBlockNotifier::NotifyBlock(const CMiniBlockIndex *pindex)
{
    uint256 hash = pindex->GetBlockHash();
    LogPrint("zmq", "zmq: Publish hashminiblock %s\n", hash.GetHex());
    LogPrintf("[%s] zmq: Publish hashminiblock %s\n", __func__, hash.GetHex());
    char data[32];
    for (unsigned int i = 0; i < 32; i++)
        data[31 - i] = hash.begin()[i];
