...
IS_SIGNED () {
	...
	if [ "$NO_SHA1" = "1" ]; then
		export ILCOIN_VERIFY_COMMITS_ALLOW_SHA1=0
	else
		export ILCOIN_VERIFY_COMMITS_ALLOW_SHA1=1
	fi

	if [ "${REVSIG_ALLOWED#*$1}" != "$REVSIG_ALLOWED" ]; then
		export ILCOIN_VERIFY_COMMITS_ALLOW_REVSIG=1
	else
		export ILCOIN_VERIFY_COMMITS_ALLOW_REVSIG=0
	fi
