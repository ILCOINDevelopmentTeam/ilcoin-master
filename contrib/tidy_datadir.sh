#!/bin/bash
# Copyright (c) 2013 The Ilcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Ilcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Ilcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Ilcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Ilcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Ilcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Ilcoin 0.8 datadir."
    ;;
esac
