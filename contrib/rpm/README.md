...
The RPM spec file provided here is for Ilcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://ilcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Ilcoin git repository](https://github.com/ilcoin/ilcoin) but are not part of
...
Currently only `Source10` is used, for the example `ilcoin.conf` file.
...
in the ilcoin git repository. At present only an SVG version of the Ilcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the ilcoin
...
needed when 0.12.1 is released, a proper fix is already in the Ilcoin git
master branch.
...
idea to build Ilcoin Core against a static version of that library compiled
...
The `ilcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `ilcoin-server` package. At this point, most RPM based
...
The `ilcoin-server` package should have SELinux support. How to properly do
...
Ilcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/ilcoin/). He has been
packaging Ilcoin for Fedora at least since 2012.
...
4. I renamed the `ilcoin` package that contains the Qt GUI to `ilcoin-core` as
that appears to be how the general population refers to it, in contrast to
`ilcoin-xt` or `ilcoin-classic`. I wanted to make sure the general population
